...
function fctfClick(){
	...
	//学习资源视频点击事件
	$(".fctf_video").live("click",function(){
		videoShow();
 ...
 * @param ceid
 */
function queryFctfExamInfo(){
	$.ajax({
		url:"newStudyPageAction!queryFctfExamInfo.action",
		data:{ceid:ceid},
		type:"post",
		dataType:"json",
		async : true,
		success:function(data){
			//下面进行数据渲染
			if($(data).attr("result") == "success"){
				var fctfStatus = $(data).attr("fctfState");
				var mes = $(data).attr("message");
				if(fctfStatus == 0){ //实验未开放
					if(mes == "noInstance"){
				...
				}else if(fctfStatus == 1 || fctfStatus == 2){//实验进行中或已完成
					if(mes == "noInstance"){
						//没有实例 询问是否需要答题
						if(testNum > 0){
							showIsStartQue();
...
function createStuExam(){
	$.ajax({
		url:"newStudyPageAction!createStuFctftExam.action",
		data:{ceid:ceid},
		type:"post",
		dataType:"json",
		async : true,
...
function showQuestionContent(objData){
	...
	if(type == 0){
		...
		str += "<p class=\"issuecontent\"><span>"+orderNo+"、<img class=\"issueType\" src=\"../../img/fctfanipulator/radio-icon.png\"/></span>"+queContent+"</p>";
		...
		var str = "<div class=\"questionsdiv\">";
		str += "<p class=\"issuecontent\"><span>"+orderNo+"、<img class=\"issueType\" src=\"../../img/fctfanipulator/multiple-icon.png\"/></span>"+queContent+"</p>";
		str += "<ul class=\"answerlist\">";
		$(optionList).each(function(i){
		...
		var str = "<div class=\"questionsdiv\">";
		str += "<p class=\"issuecontent\"><span>"+orderNo+"、<img class=\"issueType\" src=\"../../img/fctfanipulator/filling-icon.png\"/></span>"+queContent+"</p>";
...
function loadAllQue(obj){
	if(obj != null && obj != ""){
		...
		$(qlist).each(function(){
			...
			//渲染题目
			if(type == 0){
				str += "<p class=\"issuecontent\"><span>"+orderNo+"、<img class=\"issueType\" src=\"../../img/fctfanipulator/radio-icon.png\"/></span>"+content+"</p>";
			}else if(type == 1){
				str += "<p class=\"issuecontent\"><span>"+orderNo+"、<img class=\"issueType\" src=\"../../img/fctfanipulator/multiple-icon.png\"/></span>"+content+"</p>";
			}else if(type == 2){
				str += "<p class=\"issuecontent\"><span>"+orderNo+"、<img class=\"issueType\" src=\"../../img/fctfanipulator/filling-icon.png\"/></span>"+content+"</p>";
			}
			//渲染是否正确
			if(right == 1){
				str += "<img class=\"rightIcon\" src=\"../../img/fctfanipulator/right.png\"/>";
			}
			if(type != 2){ //非填空题
				//渲染答案
...
function getAssContentVray(userPicture,createTime,ecid,assessmentId,userId,assessment,fctfName,risk,replyNum,userName,replyFlag){
	...
	if(languageFirst == "English"){
		...
		replyStr += "<span class=\"evaluate_source\" id=\""+ecid+"\">stem from：<a href=\"fctfc.do?ec="+ecid+"\" target=\"_blank\">"+fctfName+"</a></span>";
	}else{
		replyStr += "<span class=\"evaluate_time\">时间："+dateStrTime(sqlDate2Str(createTime))+"</span>&nbsp;&nbsp;";
		replyStr += "<span class=\"evaluate_source\" id=\""+ecid+"\">源自：<a href=\"fctfc.do?ec="+ecid+"\" target=\"_blank\">"+fctfName+"</a></span>";
	}
	if(replyNum == 0){
 ...
 * @returns
 */
function fctfAssShow(ceid){
	var date="";
	$.ajax({
		url:"courseFctf!fctfAssShow.action",
		data:{ceid:ceid},
		type:"post",
		dataType:"json",
		async : false,
	...
	$.ajax({
		...
		success:function(data){
			if($(data).attr("result") == "success") {
				...
				var cont = parseInt($(".fctf_menu_list #evaluateBeta span span").text());
				$(".fctf_menu_list #evaluateBeta span span").text(cont+1);
				queryBetaByCeidAndUserId(ceid);
				queryBetaByEcid(ecid,1,true);
			}else{
 ...
 * @returns
 */
function fctfBetaAssShow(ceid){
	var date;
	$.ajax({
		url:"courseFctf!fctfBetaAssShow.action",
		data:{ceid:ceid},
		type:"post",
		dataType:"json",
		async : false,
...
function BetaVray(createTime,headImg,userName,userId,allEvaluation,billId,countReply){
	...
	str += "<a href=\"/pages/fctfBetaEnv.html?billId="+billId+"\" target=\"_blank\" ><div class=\"evaluateBeta_content\">"+allEvaluation+"</div></a>";
...
function addQuizs(content,ceid){
	...
	$.ajax({
		...
		success : function(data){
			...
			var mes = $(data).attr("message");
			if($(data).attr("result") == "success"){
				var cont = parseInt($(".fctf_menu_list #questions span span").text());
				$(".fctf_menu_list #questions span span").text(cont+1);
...
function addNotes(content,ceid){
	...
	$.ajax({
		...
		data:{content:content,fctftId:ceid,code:$("#notes_editor_text").data("isNeed")},
		type:"post",
		dataType:"json",
		async : true,
		success : function(data){
			...
			var mes = $(data).attr("message");
			if($(data).attr("result") == "success"){
				var cont = parseInt($(".fctf_menu_list #notes span span").text());
				$(".fctf_menu_list #notes span span").text(cont+1);
				queryNotesByEcid(ecid,1,true);
				$("#popup_cancel").trigger('click');
			}else if($(data).attr("result") == "needCode"){
				jCode("", "", function(val){
...
function queryVideoByEcid(ecid,ceid){
	$.ajax({
		url:"newFctf!queryVideoByEcid.action",
		...
		success : function(data){
			if($(data).attr("result") == "success"){
				var mes = $(data).attr("message");
				if(mes != null){
					var str = "<img class=\"fctf_video\" src=\"/img/coursePage/vedioBtn.png\">";
					$(".fctf_img_list").append(str);
					$(".videoList").empty();
					$(mes).each(function(i){
						var fctfName = $(this).attr("ecName");
						var provider = $(this).attr("userName");
						var videoNum = $(this).attr("views");
						var wareId = $(this).attr("wareId");
						var flagShow = $(this).attr("flagShow");
						var str = "<p id=\""+wareId+"\" data-provider=\""+provider+"\" data-videoNum=\""+videoNum+"\" data-name=\""+fctfName+"\">"+fctfName+"</p>";
