...
public class MigrateTask38 {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MigrateTask38.class);
        ...
        if(ser_mem_current >= ser_mem_top || ser_cpu_current >= ser_cpu_top){
            ...
            if(!sourceIp.equals(targetNodeIp)){
                ...
                if(targetVm != null){
                    ...
                    result = remoteShellExecutorMaster.exec(updateSer_cmd);
                    if(result.getRet() != -1){
                        String key = sourceVM.getName();
                        String sourseValue = redisService.get(ExpKey.existARunningVm, key, String.class);
                        String targetValue = sourseValue.replace(sourceIp, targetNodeIp);
    ...
    private Domain selectVM() throws Exception {
        ...
        Connect conn = sourceConn.getConn(ServerInfoUtil.VmServerIp38);
        try{
