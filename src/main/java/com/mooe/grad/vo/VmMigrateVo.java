...
public class VmMigrateVo {
	...
	public VmMigrateVo(int id, Domain domain) {
		this.id = id;
		this.domain = domain;
		try {
			this.numberOfPes = domain.getMaxVcpus();
			this.ram = (int) domain.getMaxMemory();
			this.bw = domain
	 ...
	 * 
	 * @param host HostMigrateInVo running the VM
	 * @pre host != $null
	 * @post $none
	 */
	...
	public void setHost(HostMigrateInVo host) {
		this.host = host;
	}
	 ...
	 * @return the host
	 */
	public HostMigrateInVo getHost() {
		return host;
	}
